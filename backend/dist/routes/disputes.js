...
const confirmPaymentSchema = joi_1.default.object({
    confirmation_type: joi_1.default.string().valid('PAYMENT_SENT', 'PAYMENT_RECEIVED').required()
});
const createAppealSchema = joi_1.default.object({
    // dispute_type: Joi.string().valid('PAYMENT_NOT_RECEIVED', 'PAYMENT_NOT_SENT', 'OTHER').required(),
    dispute_type: joi_1.default.string().optional(),
    description: joi_1.default.string().min(10).max(1000).required(),
    evidence_video_url: joi_1.default.string().optional().allow(null),
    evidence_screenshots: joi_1.default.array().items(joi_1.default.string().uri()).optional().allow(null),
    evidence_documents: joi_1.default.array().items(joi_1.default.string().uri()).optional().allow(null)
});
...
router.post('/:orderId/confirm-payment-sent', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const confirmationResult = await database_1.default.query(`INSERT INTO payment_confirmations (order_id, buyer_confirmed, buyer_confirmed_at)
       VALUES ($1, true, $2)
       ...
       RETURNING *`, [orderId, new Date()]);
        ...
        if (confirmation.seller_confirmed) {
            ...
            await database_1.default.query('UPDATE orders SET state = $1 WHERE id = $2', ['RELEASED', orderId]);
            await database_1.default.query(`INSERT INTO dispute_timeline (dispute_id, order_id, event_type, event_description, created_by)
         VALUES (NULL, $1, 'BOTH_CONFIRMED', 'Both buyer and seller confirmed payment', 'SYSTEM')`, [orderId]);
            return res.json({
                success: true,
                data: {
        ...
        // Log timeline event
        await database_1.default.query(`INSERT INTO dispute_timeline (dispute_id, order_id, event_type, event_description, created_by)
       VALUES (NULL, $1, 'BUYER_CONFIRMED', 'Buyer confirmed payment sent', $2)`, [orderId, user.address]);
        res.json({
            success: true,
            data: {
                orderId: parseInt(orderId),
...
router.post('/:orderId/confirm-payment-received', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = req.params.orderId;
        const user = req.user;
        ...
        // Create or update payment confirmation
        const confirmationResult = await database_1.default.query(`INSERT INTO payment_confirmations (order_id, seller_confirmed, seller_confirmed_at)
       VALUES ($1, true, $2)
       ON CONFLICT (order_id) 
       DO UPDATE SET 
       ...
       RETURNING *`, [orderId, new Date()]);
        ...
        if (confirmation.buyer_confirmed) {
            // Both confirmed
            await database_1.default.query(`INSERT INTO dispute_timeline (dispute_id, order_id, event_type, event_description, created_by)
         VALUES (NULL, $1, 'BOTH_CONFIRMED', 'Both buyer and seller confirmed payment. Funds released on blockchain.', 'SYSTEM')`, [orderId]);
        }
        else {
            // Seller confirmed (buyer confirmation is off-chain only)
            await database_1.default.query(`INSERT INTO dispute_timeline (dispute_id, order_id, event_type, event_description, created_by)
         VALUES (NULL, $1, 'SELLER_CONFIRMED', 'Seller confirmed payment received and released funds on blockchain', $2)`, [orderId, user.address]);
        }
...
router.post('/:orderId/appeal', upload.fields([
    ...
    try {
        ...
        let disputeResult = await database_1.default.query('SELECT * FROM disputes WHERE order_id = $1', [orderId]);
        let disputeId;
        if (disputeResult.rows.length === 0) {
            const newDisputeResult = await database_1.default.query(`INSERT INTO disputes (order_id, dispute_type, created_by, status)
         VALUES ($1, $2, $3, 'PENDING')
         RETURNING *`, [orderId, value.dispute_type, appellant_address]);
            disputeId = newDisputeResult.rows[0].id;
            // Update order state
            await database_1.default.query('UPDATE orders SET state = $1, dispute_id = $2 WHERE id = $3', ['UNDER_DISPUTE', disputeId, orderId]);
         ...
         VALUES ($1, $2, 'NEW_DISPUTE', 'New Dispute Filed', 'A new dispute has been filed for order #' || $2, 'HIGH')`, [disputeId, orderId]);
        }
        else {
            disputeId = disputeResult.rows[0].id;
        }
        // Check if appellant has already filed an appeal
        const existingAppeal = await database_1.default.query('SELECT * FROM appeals WHERE dispute_id = $1 AND appellant_address = $2', [disputeId, appellant_address]);
        ...
        // Create appeal
        const appealResult = await database_1.default.query(`INSERT INTO appeals (dispute_id, order_id, appellant_address, appellant_type, description, evidence_video_url, evidence_screenshots, evidence_documents)
...
// Get dispute status
router.get('/:orderId/status', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = req.params.orderId;
        const user = req.user;
        // Get order details
        ...
        const confirmationResult = await database_1.default.query('SELECT * FROM payment_confirmations WHERE order_id = $1', [orderId]);
        // Get dispute details if exists
        const disputeResult = await database_1.default.query('SELECT * FROM disputes WHERE order_id = $1', [orderId]);
        // Get appeals if dispute exists
        let appeals = [];
        if (disputeResult.rows.length > 0) {
            const appealsResult = await database_1.default.query('SELECT * FROM appeals WHERE dispute_id = $1 ORDER BY created_at DESC', [disputeResult.rows[0].id]);
