...
router.delete('/:id', auth_1.authenticateAdmin, async (req, res) => {
    try {
        ...
        const agentCheck = await database_1.pool.query('SELECT COUNT(*) as agent_count FROM agents WHERE location_id = $1', [id]);
        ...
        const result = await database_1.pool.query('DELETE FROM locations WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Location not found'
