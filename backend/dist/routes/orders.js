...
const createOrderSchema = joi_1.default.object({
    ad_id: joi_1.default.number().integer().positive().required(),
    amount: joi_1.default.number().positive().required(),
    selected_agent_id: joi_1.default.string().optional(),
    timezone: joi_1.default.string().required() // User's timezone for display purposes only
});
const acceptOrderSchema = joi_1.default.object({});
// Create order
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const adResult = await database_1.default.query(`SELECT a.*, u.address as owner_address, ag.branch_name, ag.mobile as agent_mobile, ag.address as agent_address
       FROM ads a
       JOIN users u ON a.owner_address = u.address
       LEFT JOIN agents ag ON a.owner_selected_agent_id = ag.id
       WHERE a.id = $1 AND a.active = true`, [orderData.ad_id]);
        ...
        if (orderData.selected_agent_id) {
            ...
            const agentResult = await database_1.default.query('SELECT * FROM agents WHERE id = $1', [orderData.selected_agent_id]);
        ...
        const orderResult = await database_1.default.query(`INSERT INTO orders (ad_id, buyer_address, seller_address, amount, token, agent_branch, agent_number, agent_address, timezone)
       ...
       RETURNING *`, [
        ...
        await database_1.default.query(`UPDATE orders SET start_datetime_string = $1 WHERE id = $2`, [startDatetimeString, order.id]);
        // Refresh order data to get the updated start_datetime_string
        const updatedOrderResult = await database_1.default.query(`SELECT * FROM orders WHERE id = $1`, [order.id]);
        ...
        await (0, orderWorker_1.scheduleOrderTimeout)(finalOrder.id);
        // Log order creation
        console.log('📝 CREATE ORDER: Logging audit trail');
        await database_1.default.query('INSERT INTO audit_logs (user_address, action, resource_type, resource_id, details, ip_address) VALUES ($1, $2, $3, $4, $5, $6)', [user.address, 'CREATE_ORDER', 'order', finalOrder.id.toString(), JSON.stringify({ ad_id: orderData.ad_id, amount: orderData.amount, expires_at: expiresAt }), req.ip]);
        const response = {
            success: true,
            data: {
                ...finalOrder,
...
router.post('/:id/prepare-accept', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // Get order details
        const orderResult = await database_1.default.query(`SELECT o.*, a.owner_address as ad_owner_address, a.type as ad_type
       FROM orders o
       ...
       WHERE o.id = $1`, [orderId]);
        ...
        await database_1.default.query(`INSERT INTO otp_logs (order_id, user_address, otp_hash, expires_at, used)
       VALUES ($1, $2, $3, $4, false)`, [orderId, user.address, otpHash, otpExpiresAt]);
    ...
    try {
       ...
       WHERE o.id = $1`, [orderId]);
        ...
        try {
         ...
         WHERE id = $7`, [acceptedAt, lockExpiresAt, otpHash, txHash, blockchainTradeId, createTradeTxHash, orderId]);
            // Generate transaction record
            console.log('💳 ACCEPT ORDER: Creating transaction record');
            const transactionNumber = `TXN-${Date.now()}-${orderId}`;
            const adResult = await client.query('SELECT type, token, sell_quantity, buy_quantity FROM ads WHERE id = $1', [order.ad_id]);
            if (adResult.rows.length === 0) {
          ...
          amount, token, transaction_type, status
            ...
            if (ad.type === 'SELL' && ad.sell_quantity !== null) {
                ...
                await client.query(`UPDATE ads SET sell_quantity = $1, updated_at = NOW() WHERE id = $2`, [newQuantity.toString(), order.ad_id]);
                ...
                if (newQuantity <= 0) {
                    await client.query(`UPDATE ads SET active = false, updated_at = NOW() WHERE id = $1`, [order.ad_id]);
                ...
                await client.query(`UPDATE ads SET buy_quantity = $1, updated_at = NOW() WHERE id = $2`, [newQuantity.toString(), order.ad_id]);
        ...
        await database_1.default.query('INSERT INTO audit_logs (user_address, action, resource_type, resource_id, ip_address) VALUES ($1, $2, $3, $4, $5)', [user.address, 'ACCEPT_ORDER', 'order', orderId, req.ip]);
        const response = {
            success: true,
            data: {
                orderId: parseInt(orderId),
...
router.post('/:id/accept-simple', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = req.params.id;
        const user = req.user;
        ...
        const adResult = await database_1.default.query('SELECT owner_address FROM ads WHERE id = $1', [order.ad_id]);
        if (adResult.rows.length === 0) {
        ...
        state = 'ACCEPTED', 
        accepted_at = $1
       WHERE id = $2`, [acceptedAt, orderId]);
        // Log acceptance
        console.log('📝 ACCEPT SIMPLE: Logging audit trail');
        await database_1.default.query('INSERT INTO audit_logs (user_address, action, resource_type, resource_id, ip_address) VALUES ($1, $2, $3, $4, $5)', [user.address, 'ACCEPT_ORDER_SIMPLE', 'order', orderId, req.ip]);
        const response = {
            success: true,
            data: {
                orderId: parseInt(orderId),
...
router.post('/:id/prepare-lock', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = req.params.id;
        const user = req.user;
        ...
        // Generate OTP
        const otp = Math.floor(100000 + Math.random() * 900000).toString();
        const otpHash = '0x' + crypto_1.default.createHash('sha256').update(otp).digest('hex');
...
router.post('/:id/lock-funds', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = req.params.id;
        const user = req.user;
        const { txHash, otpHash, blockchainTradeId, createTradeTxHash } = req.body;
        ...
        const updateResult = await database_1.default.query(`UPDATE orders SET 
        state = 'LOCKED', 
        lock_expires_at = $1,
        otp_hash = $2,
        tx_hash = $3,
       ...
       WHERE id = $6
       RETURNING blockchain_trade_id`, [lockExpiresAt, otpHash, txHash, tradeIdNumber, createTradeTxHash, orderId]);
        ...
        const transactionNumber = `TXN-${Date.now()}-${orderId}`;
        const adResult = await database_1.default.query('SELECT type, token, sell_quantity, buy_quantity FROM ads WHERE id = $1', [order.ad_id]);
        if (adResult.rows.length === 0) {
            throw new Error('Ad not found for transaction creation');
        }
        const ad = adResult.rows[0];
        ...
        if (ad.type === 'SELL' && ad.sell_quantity !== null) {
            const newQuantity = Math.max(0, parseFloat(ad.sell_quantity) - parseFloat(order.amount));
            await database_1.default.query(`UPDATE ads SET sell_quantity = $1, updated_at = NOW() WHERE id = $2`, [newQuantity.toString(), order.ad_id]);
            console.log(`📉 LOCK FUNDS: Reduced sell_quantity from ${ad.sell_quantity} to ${newQuantity} for ad ${order.ad_id}`);
            // If quantity reaches 0, deactivate the ad
            if (newQuantity <= 0) {
                await database_1.default.query(`UPDATE ads SET active = false, updated_at = NOW() WHERE id = $1`, [order.ad_id]);
                console.log(`🚫 LOCK FUNDS: Deactivated ad ${order.ad_id} (quantity exhausted)`);
            }
        }
        else if (ad.type === 'BUY' && ad.buy_quantity !== null) {
            const newQuantity = Math.max(0, parseFloat(ad.buy_quantity) - parseFloat(order.amount));
            await database_1.default.query(`UPDATE ads SET buy_quantity = $1, updated_at = NOW() WHERE id = $2`, [newQuantity.toString(), order.ad_id]);
        ...
        await database_1.default.query('INSERT INTO audit_logs (user_address, action, resource_type, resource_id, details, ip_address) VALUES ($1, $2, $3, $4, $5, $6)', [user.address, 'LOCK_FUNDS', 'order', orderId, JSON.stringify({ txHash, blockchainTradeId: tradeIdNumber }), req.ip]);
        const response = {
            success: true,
            data: {
                orderId: parseInt(orderId),
...
// Verify payment
router.post('/:id/verify-payment', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = req.params.id;
        const user = req.user;
        ...
        try {
            ...
            await database_1.default.query('UPDATE orders SET state = $1, tx_hash = $2 WHERE id = $3', ['RELEASED', txHash, orderId]);
            await database_1.default.query('INSERT INTO audit_logs (user_address, action, resource_type, resource_id, details, ip_address) VALUES ($1, $2, $3, $4, $5, $6)', [user.address, 'VERIFY_PAYMENT', 'order', orderId, JSON.stringify({ txHash }), req.ip]);
            const response = {
                success: true,
                data: {
                    orderId: parseInt(orderId),
...
router.post('/:id/cancel', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        await database_1.default.query('UPDATE orders SET state = $1 WHERE id = $2', ['CANCELLED', orderId]);
        await database_1.default.query('INSERT INTO audit_logs (user_address, action, resource_type, resource_id, details, ip_address) VALUES ($1, $2, $3, $4, $5, $6)', [user.address, 'CANCEL_ORDER', 'order', orderId, JSON.stringify({ reason }), req.ip]);
        res.json({
            success: true,
...
router.get('/requests', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`SELECT o.*, 
              a.type as ad_type, 
              a.token, 
              a.price_inr, 
       ...
       ORDER BY o.created_at DESC`, [user.address]);
        const serverTime = new Date().toISOString();
        const serverTimeMs = Date.now();
