...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        const reviewSchema = joi_1.default.object({
            reviewee_address: joi_1.default.string().required(),
            order_id: joi_1.default.number().integer().optional(),
            rating: joi_1.default.number().integer().min(1).max(5).required(),
            message: joi_1.default.string().max(500).optional().allow('')
        });
      ...
      `, [normalizedReviewerAddress, order_id]);
            ...
            const existingGeneralReview = await database_1.default.query(`
        SELECT id FROM reviews 
