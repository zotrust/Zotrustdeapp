...
const updateProfileSchema = joi_1.default.object({
    ...
    location_id: joi_1.default.number().integer().positive().optional(),
    selected_agent_ids: joi_1.default.array().items(joi_1.default.number().integer().positive()).optional()
});
// Update profile
router.put('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (profileData.selected_agent_ids && profileData.selected_agent_ids.length > 0) {
            const agentResult = await database_1.default.query('SELECT * FROM agents WHERE id = ANY($1) AND verified = true', [profileData.selected_agent_ids]);
            ...
            if (profileData.location_id) {
                const locationResult = await database_1.default.query('SELECT DISTINCT location_id FROM agents WHERE id = ANY($1)', [profileData.selected_agent_ids]);
        ...
        // Log profile update
        await database_1.default.query('INSERT INTO audit_logs (user_address, action, resource_type, details, ip_address) VALUES ($1, $2, $3, $4, $5)', [user.address, 'UPDATE_PROFILE', 'user', profileData, req.ip]);
...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`SELECT u.*, 
              COALESCE(
                (SELECT json_agg(
                  json_build_object(
                    'id', a.id,
       ...
       FROM users u
       WHERE u.address = $1`, [user.address]);
        const response = {
            success: true,
            data: result.rows[0]
        };
