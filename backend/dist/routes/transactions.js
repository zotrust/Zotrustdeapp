...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await database_1.pool.query(`SELECT t.*, o.amount, o.token, o.state as order_state,
       ...
       LIMIT $2 OFFSET $3`, [user.address, parseInt(limit), parseInt(offset)]);
...
router.get('/:id', async (req, res) => {
    try {
       ...
       WHERE t.id = $1 AND (t.buyer_address = $2 OR t.seller_address = $2)`, [id, user.address]);
