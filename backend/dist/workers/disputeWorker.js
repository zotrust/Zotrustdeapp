...
exports.scheduleDisputeProcessing = exports.processDisputeTimeouts = void 0;
...
const processDisputeTimeouts = async () => {
    try {
        ...
        const expiredOrdersResult = await database_1.default.query(`
    ...
    `);
        ...
        const expiredAppealsResult = await database_1.default.query(`
...
exports.processDisputeTimeouts = processDisputeTimeouts;
// Process individual expired order
const processExpiredOrder = async (order) => {
    try {
        console.log(`🔄 DISPUTE WORKER: Processing expired order ${order.id}`);
        // Check if dispute already exists
        const existingDispute = await database_1.default.query('SELECT * FROM disputes WHERE order_id = $1', [order.id]);
        if (existingDispute.rows.length > 0) {
            console.log(`⏭️ DISPUTE WORKER: Dispute already exists for order ${order.id}`);
            return;
        }
        // Create dispute automatically
        const disputeResult = await database_1.default.query(`INSERT INTO disputes (order_id, dispute_type, created_by, status)
       VALUES ($1, 'PAYMENT_NOT_RECEIVED', 'SYSTEM', 'PENDING')
       RETURNING *`, [order.id]);
        const disputeId = disputeResult.rows[0].id;
        // Update order state
        await database_1.default.query('UPDATE orders SET state = $1, dispute_id = $2, appeal_deadline = $3 WHERE id = $4', ['UNDER_DISPUTE', disputeId, new Date(Date.now() + (48 * 60 * 60 * 1000)), order.id]);
        // Log timeline event
       ...
       VALUES ($1, $2, 'AUTO_DISPUTE', 'Order Auto-Disputed', $3, 'HIGH')`, [disputeId, order.id, notificationMessage]);
...
const processExpiredAppeal = async (order) => {
    try {
        console.log(`🔄 DISPUTE WORKER: Processing expired appeal for order ${order.id}`);
        // Check if any appeals were filed
        const appealsResult = await database_1.default.query('SELECT * FROM appeals WHERE dispute_id = $1', [order.dispute_id]);
        ...
        try {
         ...
         WHERE id = $2`, [new Date(), order.dispute_id]);
            // Update order state
            await database_1.default.query('UPDATE orders SET state = $1, tx_hash = $2 WHERE id = $3', ['REFUNDED', txHash, order.id]);
            // Log timeline event
            await database_1.default.query(`INSERT INTO dispute_timeline (dispute_id, order_id, event_type, event_description, created_by, metadata)
...
const scheduleDisputeProcessing = () => {
    // Run immediately on startup
    (0, exports.processDisputeTimeouts)();
    // Schedule to run every 5 minutes
    setInterval(exports.processDisputeTimeouts, 5 * 60 * 1000);
    console.log('⏰ DISPUTE WORKER: Scheduled dispute processing (every 5 minutes)');
};
exports.scheduleDisputeProcessing = scheduleDisputeProcessing;
