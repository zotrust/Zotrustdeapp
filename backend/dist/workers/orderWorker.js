...
exports.scheduleExpiredOrdersCheck = exports.scheduleOTPCleanup = exports.removeScheduledJob = exports.scheduleLockExpiry = exports.scheduleOrderTimeout = void 0;
...
const scheduleOrderTimeout = async (orderId) => {
...
exports.scheduleOrderTimeout = scheduleOrderTimeout;
// Schedule lock expiry check
const scheduleLockExpiry = async (orderId, lockExpiresAt) => {
...
exports.scheduleLockExpiry = scheduleLockExpiry;
...
const processOrderTimeout = async (orderId) => {
    let client = null;
    try {
        const currentIST = getCurrentIST();
        ...
        if (order.state === 'CREATED') {
            await client.query('UPDATE orders SET state = $1 WHERE id = $2', ['CANCELLED', orderId]);
            // Log automatic cancellation with timezone info
            await client.query('INSERT INTO audit_logs (action, resource_type, resource_id, details) VALUES ($1, $2, $3, $4)', ['AUTO_CANCEL_ORDER', 'order', orderId.toString(), JSON.stringify({
...
// Process lock expiry
const processLockExpiry = async (orderId) => {
    let client = null;
    try {
        const currentIST = getCurrentIST();
        ...
        if (['ACCEPTED', 'LOCKED'].includes(order.state)) {
            ...
            await client.query('UPDATE orders SET state = $1 WHERE id = $2', ['UNDER_DISPUTE', orderId]);
            // Log lock expiry with timezone info
            await client.query('INSERT INTO audit_logs (action, resource_type, resource_id, details) VALUES ($1, $2, $3, $4)', ['AUTO_DISPUTE_ORDER', 'order', orderId.toString(), JSON.stringify({
                    reason: 'Lock expired',
...
exports.scheduleOTPCleanup = scheduleOTPCleanup;
...
const scheduleExpiredOrdersCheck = () => {
    const currentIST = getCurrentIST();
...
exports.scheduleExpiredOrdersCheck = scheduleExpiredOrdersCheck;
    ...
    try {
       ...
       RETURNING id, buyer_address, seller_address, created_at`);
        if (result.rowCount && result.rowCount > 0) {
            console.log(`⏰ Marked ${result.rowCount} orders as EXPIRED at ${formatISTDateTime(currentIST)} (IST):`, result.rows.map(r => `#${r.id}`).join(', '));
            // Log each expired order with timezone info
            for (const order of result.rows) {
                try {
                    const orderCreatedIST = toIST(new Date(order.created_at));
                    await client.query('INSERT INTO audit_logs (action, resource_type, resource_id, details) VALUES ($1, $2, $3, $4)', [
                        'AUTO_EXPIRE_ORDER',
                        'order',
                        order.id.toString(),
                        JSON.stringify({
