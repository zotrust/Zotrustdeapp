...
router.get('/appeals', authenticateAdmin, async (req, res) => {
  try {
...
router.get('/disputes', authenticateAdmin, async (req, res) => {
  try {
    ...
    const disputesWithAppeals = await Promise.all(
...
// Get dispute details
router.get('/disputes/:disputeId', authenticateAdmin, async (req, res) => {
  try {
...
router.post('/disputes/:disputeId/resolve', authenticateAdmin, async (req, res) => {
  try {
    const disputeId = req.params.disputeId;
    ...
    try {
      ...
      await pool.query(
        `UPDATE disputes SET 
          ...
          resolution_reason = $4
         WHERE id = $5`,
        [new Date(), (req as any).user.address, value.resolution, value.resolution_reason, disputeId]
      );

      // Update order state
      await pool.query(
        'UPDATE orders SET state = $1, tx_hash = $2 WHERE id = $3',
        [newOrderState, txHash, dispute.order_id]
      );
...
router.post('/notifications/:notificationId/read', authenticateAdmin, async (req, res) => {
  try {
    const notificationId = req.params.notificationId;

    await pool.query(
      'UPDATE admin_notifications SET status = $1, read_at = $2 WHERE id = $3',
      ['READ', new Date(), notificationId]
    );
...
// Resolve appeal
router.post('/appeals/:appealId/resolve', authenticateAdmin, async (req, res) => {
  try {
    ...
    await pool.query(
      'UPDATE appeals SET status = $1, resolved_at = NOW() WHERE id = $2',
      ['RESOLVED', appealId]
    );

    // Update dispute status
    await pool.query(
      'UPDATE disputes SET status = $1, resolution = $2, resolution_reason = $3, resolved_at = $4, resolved_by = $5 WHERE id = $6',
      ['RESOLVED', value.resolution, value.resolution_reason, new Date(), (req as any).user?.address || 'ADMIN', appeal.dispute_id]
    );
...
router.get('/disputes/stats', authenticateAdmin, async (req, res) => {
  try {
...
router.post('/orders/:orderId/sync-blockchain-status', authenticateAdmin, async (req, res) => {
  try {
    ...
    function getBlockchainStatusName(status: number): string {
