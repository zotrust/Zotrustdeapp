...
router.get('/test-token', authenticateAdmin, async (req, res) => {
  try {
    res.json({
      success: true,
...
router.get('/dashboard-stats', authenticateAdmin, async (req, res) => {
  try {
    ...
    // Get total disputes
    const disputesResult = await pool.query('SELECT COUNT(*) as count FROM disputes');
    ...
    const completedOrdersResult = await pool.query(
      'SELECT COUNT(*) as count FROM orders WHERE state = $1',
      ['RELEASED']
    );
    ...
    const volumeResult = await pool.query(
      'SELECT COALESCE(SUM(o.amount * a.price_inr), 0) as total_volume FROM orders o JOIN ads a ON o.ad_id = a.id WHERE o.state = $1',
      ['RELEASED']
    );
...
const createAgentSchema = Joi.object({
  branch_name: Joi.string().max(255).required(),
...
const updateAgentSchema = Joi.object({
  branch_name: Joi.string().max(255).optional(),
...
const setFeeWalletSchema = Joi.object({
...
router.post('/agents', authenticateAdmin, async (req, res) => {
  try {
    ...
    const locationResult = await pool.query(
      'SELECT city FROM locations WHERE id = $1',
    ...
    const result = await pool.query(
      `INSERT INTO agents (branch_name, city, mobile, address, location_id, verified, created_by_admin)
    ...
    // Log agent creation
    await pool.query(
      'INSERT INTO audit_logs (action, resource_type, resource_id, details, ip_address) VALUES ($1, $2, $3, $4, $5)',
      ['ADMIN_CREATE_AGENT', 'agent', result.rows[0].id.toString(), JSON.stringify(agentData), req.ip]
    );
...
// Update agent
router.patch('/agents/:id', authenticateAdmin, async (req, res) => {
  try {
    ...
    await pool.query(
      'INSERT INTO audit_logs (action, resource_type, resource_id, details, ip_address) VALUES ($1, $2, $3, $4, $5)',
      ['ADMIN_UPDATE_AGENT', 'agent', agentId, JSON.stringify(value), req.ip]
    );
...
router.delete('/agents/:id', authenticateAdmin, async (req, res) => {
  try {
    ...
    const adsResult = await pool.query(
      'SELECT COUNT(*) as count FROM ads WHERE owner_selected_agent_id = $1 AND active = true',
      [agentId]
    );
    ...
    const usersResult = await pool.query(
      'SELECT COUNT(*) as count FROM users WHERE selected_agent_id = $1',
      [agentId]
    );
    ...
    // Log agent deletion
    await pool.query(
      'INSERT INTO audit_logs (action, resource_type, resource_id, ip_address) VALUES ($1, $2, $3, $4)',
      ['ADMIN_DELETE_AGENT', 'agent', agentId, req.ip]
    );
...
router.post('/orders/:id/refund', authenticateAdmin, async (req, res) => {
  try {
    ...
    try {
      ...
      await pool.query(
        'INSERT INTO audit_logs (action, resource_type, resource_id, details, ip_address) VALUES ($1, $2, $3, $4, $5)',
        ['ADMIN_REFUND_ORDER', 'order', orderId, JSON.stringify({ reason, txHash }), req.ip]
      );
...
router.post('/set-fee-wallet', authenticateAdmin, async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      ['ADMIN_SET_FEE_WALLET', JSON.stringify({ address }), req.ip]
    );
...
router.put('/trading-hours', authenticateAdmin, async (req, res) => {
  try {
    ...
    await pool.query(
      'INSERT INTO audit_logs (action, details, ip_address) VALUES ($1, $2, $3)',
      ['ADMIN_UPDATE_TRADING_HOURS', JSON.stringify({ buyEnabled, sellEnabled, startTime, endTime }), req.ip]
    );
...
router.get('/dashboard', authenticateAdmin, async (req, res) => {
  try {
    const [
      ...
      pool.query('SELECT COUNT(*) as count FROM users WHERE verified = true'),
      ...
      pool.query('SELECT COUNT(*) as count FROM ads WHERE active = true'),
      pool.query('SELECT COUNT(*) as count FROM orders'),
      pool.query('SELECT COUNT(*) as count FROM orders WHERE state = \'RELEASED\'')
    ]);
...
router.post('/locations', authenticateAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO locations (name, city, state, country, active) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [name, city, state, country, active]
    );
...
router.put('/locations/:id', authenticateAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE locations SET name = $1, city = $2, state = $3, country = $4, active = $5 WHERE id = $6 RETURNING *',
      [name, city, state, country, active, id]
    );
...
router.delete('/locations/:id', authenticateAdmin, async (req, res) => {
  try {
    ...
    const agentsResult = await pool.query(
      'SELECT COUNT(*) as count FROM agents WHERE location_id = $1',
      [id]
    );
...
router.post('/agents', authenticateAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO agents (branch_name, mobile, address, location_id, verified, created_by_admin) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [branch_name, mobile, address, location_id, verified, 1]
    );
...
router.put('/agents/:id', authenticateAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE agents SET branch_name = $1, mobile = $2, address = $3, location_id = $4, verified = $5 WHERE id = $6 RETURNING *',
      [branch_name, mobile, address, location_id, verified, id]
    );
